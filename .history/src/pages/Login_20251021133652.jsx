...
const Login = ({ isDark, setIsDark }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post('http://localhost:4000/api/authlogin', {
        identifier,
        password,
      });
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-2">
              <Label htmlFor="identifier" className={isDark ? 'text-gray-200' : 'text-gray-700'}>
                Email or Username
              </Label>
              <Input
              ...
              />
              {errors.identifier && (
                <p data-testid="identifier-error" className="text-sm text-red-500 mt-1 animate-slide-in">
                  {errors.identifier}
                </p>
              )}
            </div>
